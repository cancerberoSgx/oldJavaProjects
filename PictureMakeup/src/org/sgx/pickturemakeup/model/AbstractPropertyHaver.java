		...
		if(!found)
			...
			try {
				...
				Method setter = this.getClass().getMethod(
						getSetterNameFromPropId(propId), params);
//				System.out.println("trying to invoke method "+setter+" IN "+this+" with params "+value);
//				try {
...
//				}
				...
				throw new RuntimeException("SecurityException when trying to access method "+
						getSetterNameFromPropId(propId)+" in class "+this.getClass());
			} catch (NoSuchMethodException e) {
				e.printStackTrace();
				throw new RuntimeException("NoSuchMethodException when trying to access method "+
						getSetterNameFromPropId(propId)+" in class "+this.getClass()+
						". Read AbstractPropertyHaver API comments on how to name getters and setters. ");
			} catch (IllegalArgumentException e) {
				throw new RuntimeException("IllegalArgumentException when trying to invoke method "+
						getSetterNameFromPropId(propId)+" in class "+this.getClass()+
						". Read AbstractPropertyHaver API comments on how to name getters and setters. ");
			} catch (IllegalAccessException e) {
				throw new RuntimeException("IllegalAccessException when trying to invoke method "+
						getSetterNameFromPropId(propId)+" in class "+this.getClass()+
						". Read AbstractPropertyHaver API comments on how to name getters and setters. ");
			} catch (InvocationTargetException e) {
				throw new RuntimeException("InvocationTargetException when trying to invoke method "+
						getSetterNameFromPropId(propId)+" in class "+this.getClass()+
						". Read AbstractPropertyHaver API comments on how to name getters and setters. The exception says: "+e+" - "+e.getCause());
			}
	...
	public String getSetterNameFromPropId(String propId) {
		return "set"+propId.substring(0,1).toUpperCase()+
			propId.substring(1,propId.length());
	}
	public String getGetterNameFromPropId(String propId) {
		return "get"+propId.substring(0,1).toUpperCase()+
			propId.substring(1,propId.length());
	}
	...
	public Object getPropertyValue(String propId) {
		...
		try {
			...
			Method setter = this.getClass().getMethod(
					getGetterNameFromPropId(propId), params);
			return setter.invoke(this, new Object[]{});
		} catch (SecurityException e) {
			throw new RuntimeException("SecurityException when trying to access method "+
					getGetterNameFromPropId(propId)+" in class "+this.getClass());
		} catch (NoSuchMethodException e) {
			throw new RuntimeException("SecurityException when trying to access method "+
					getGetterNameFromPropId(propId)+" in class "+this.getClass()+
					". Read AbstractPropertyHaver API comments on how to name getters and setters. ");
