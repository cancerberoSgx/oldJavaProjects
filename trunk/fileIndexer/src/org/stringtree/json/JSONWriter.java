...
//            else if (object.getClass().isArray()) array(object);
            else if (ReflectionHelper.isArray(object)) array(object);
...
//    }
    ...
    private void bean(Object object) {
        ...
        boolean addedSomething = false;
//        try {
        	Map<String, Class> props = ReflectionHelper.getProperties(object);
        	Set<String> propsNames = props.keySet();
        	for(String name : propsNames) {
        		Object value = ReflectionHelper.get(object, name);
        		 if (addedSomething) add(',');
                 add(name, value);
                 addedSomething = true;
...
//        int length = Array.getLength(object);
        int length = ReflectionHelper.arrayLength(object);
        for (int i = 0; i < length; ++i) {
//            value(Array.get(object, i));
        	 value(ReflectionHelper.arrayGet(object,i));
            if (i < length - 1) add(',');
        }
        add("]");
    }
...
//            else if (Character.isISOControl(c)) {
            else if (ReflectionHelper.characterIsIsoControl(c)) {
                unicode(c);
            } else {
                add(c);
            }
