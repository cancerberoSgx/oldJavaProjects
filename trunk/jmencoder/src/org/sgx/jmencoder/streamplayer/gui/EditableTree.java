...
/**
 * this tree let the user drag and drop items, remove with del key and edit
 * labels. This is a general widget.
 * 
 * @author sgurin
 * 
 */
public class EditableTree {
	...
	public JScrollPane getContent() {
		...
		tree.setEditable(editableLabels);
	...
	public static void main(String[] args) {
		EditableTree treeUtil = new EditableTree();
		treeUtil.setEditableLabels(true);
		JFrame f = new JFrame();
		f.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		f.add(treeUtil.getContent());
		f.setSize(400, 400);
...
class TreeTransferHandler extends TransferHandler {
	...
	public boolean canImport(TransferHandler.TransferSupport support) {
		...
		// sgurin: drag drop - move - copy actions policies. The following are examples of policies but are commented by defaiult:
		
		// Do not allow MOVE-action drops if a non-leaf node is
		// selected unless all of its children are also selected.  
//		sgurin. TODO: ideally the user doesnt want to select all childrens for moving a non 
//		leaf nodes. but if I comment the following policy an error occurs on non leaf nodes (wiThout selecting all children)
		int action = support.getDropAction();
		if (action == MOVE) {
			return haveCompleteNode(tree);
		}
		// Do not allow a non-leaf node to be copied to a level
		// which is less than its source level. sgurin: policy commented
...
//		}
	...
	/** sgurin: changed this. old javadoc said: Defensive copy used in createTransferable. */
