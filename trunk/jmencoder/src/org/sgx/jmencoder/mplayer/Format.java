...
public class Format {
	...
	public static String mencoderCommandFor(String in, String out, MediaInfo info, 
		...
		if(format.equals(XVID)) {
			return MPlayerUtils.mencoderExe+" -forceidx "+	
				subFragment+			
				Utils.quotePath(in)+	
			...
			return MPlayerUtils.mencoderExe +subFragment+	
				" -ffourcc DX50 " +
				" -vf " +
//				"scale="+info.get(MediaInfo.ID_VIDEO_WIDTH)+":"+info.get(MediaInfo.ID_VIDEO_HEIGHT)+"," +
				"harddup"+
				...
				" -o nul"+
			"\n\n"+
			MPlayerUtils.mencoderExe +subFragment+	
				" -ffourcc DX50 " +
				" -vf " +
//				"scale="+info.get(MediaInfo.ID_VIDEO_WIDTH)+":"+info.get(MediaInfo.ID_VIDEO_HEIGHT)+"," +
				"harddup"+
...
//			" -ffourcc DIV3 " +
			...
			return MPlayerUtils.mencoderExe +
					subFragment+	
		...
		else if(format.equals(Format.FLV)) {					
//			Creating a Macromedia Flash video suitable for playback in a web browser with the Macromedia Flash plugin:
		...
		else if(format.equals(Format.FFMPEG_AVI) || format.equals(Format.FFMPEG_MPEG)) {
			return MPlayerUtils.ffmpegExe+" -y -i "+Utils.quotePath(in)+" -ab "+Integer.parseInt(info.get(MediaInfo.ID_AUDIO_BITRATE,"128000"))+ 
		...
		else if(format.equals(Format.MP4)) {	
			return MPlayerUtils.mencoderExe+subFragment+
				" -vf " +
//				"scale="+info.get(MediaInfo.ID_VIDEO_WIDTH)+":"+info.get(MediaInfo.ID_VIDEO_HEIGHT)+"," +
						"harddup"+
		...
		else if(format.equals(Format.IPOD)) { //from http://ffmpeg.mplayerhq.hu/faq.html#SEC25
			return MPlayerUtils.ffmpegExe +"-i "+Utils.quotePath(in)+
				" -acodec libfaac -ab 128kb -vcodec mpeg4 -b 1200kb -mbd 2 -flags +4mv+trell -aic 2 -cmp 2 -subcmp 2 -s 320x180 -title X " +
				Utils.quotePath(out);
		}
		else if(format.equals(Format.PSP)) { //from http://ffmpeg.mplayerhq.hu/faq.html#SEC25
			return MPlayerUtils.ffmpegExe +
